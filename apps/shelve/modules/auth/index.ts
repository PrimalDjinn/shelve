...
import { getSelectedEmailProviderName } from "unemail";
...
const requiredCoreSchema = z.object({
  DATABASE_URL: z
    .url()
    .startsWith("postgres", { error: "Must be a valid PostgreSQL URL." })
    .optional(),
  NUXT_SESSION_PASSWORD: z.string().min(32, {
    error: "Session password must be at least 32 characters long.",
  }),
  NUXT_PRIVATE_ENCRYPTION_KEY: z
    .string()
    .min(32, { error: "Encryption key must be at least 32 characters long." }),
});
...
const emailSchema = z.object({
  NUXT_PRIVATE_RESEND_API_KEY: z
    .string()
    .startsWith("re_", { error: 'Resend API key must start with "re_"' })
    .optional()
    .or(z.literal("")),
  NUXT_PRIVATE_SENDER_EMAIL: z
    .email({ error: "Sender email must be a valid email address." })
    .optional()
    .or(z.literal("")),
});
...
const adminAndSecuritySchema = z.object({
  NUXT_PRIVATE_ADMIN_EMAILS: commaSeparatedStringToArray.pipe(
    z.array(z.email({ error: "One of the admin emails is invalid." }))
  ),
  NUXT_PRIVATE_ALLOWED_ORIGINS: commaSeparatedStringToArray.pipe(
    z.array(z.url({ error: "One of the allowed origins is an invalid URL." }))
  ),
});
...
export default defineNuxtModule({
  ...
  setup(options, nuxt) {
    try {
      ...
      const { result: providerName } = execute(getSelectedEmailProviderName) || env.NUXT_PRIVATE_RESEND_API_KEY;
      nuxt.options.appConfig.auth = {
        isGoogleEnabled,
        isGithubEnabled,
        isEmailEnabled: !!providerName,
      };

      console.log("🔐 Auth configuration validated:");
      console.log(`  GitHub OAuth: ${isGithubEnabled ? "✅" : "❌"}`);
      console.log(`  Google OAuth: ${isGoogleEnabled ? "✅" : "❌"}`);
      console.log(`  Email Service: ${providerName ? "✅" : "❌"}`);
    } catch (error) {
